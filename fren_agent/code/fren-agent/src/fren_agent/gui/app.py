...
def load_config_local(cfg_path: str) -> dict:
    ...
    # pass governance through unchanged if present
    if "governance" in cfg:
        out["governance"] = cfg["governance"]
    return out

class VideoThread(QtCore.QThread):
    frame_ready = QtCore.pyqtSignal(object, str)  # (numpy.ndarray frame_bgr, caption)
    ...
    def run(self):
        # Import cv2 here to avoid heavy imports at module import time
        import cv2
        while self._running:
            ...
            if ok and frame is not None:
                try:
                    caption = self.agent.vision.caption_frame(frame)
...
class FrenGUI(QtWidgets.QMainWindow):
    def __init__(self, cfg_path: str):
        ...
        # -------- load config and build agent (delayed import keeps things clean) --------
        cfg_dict = load_config_local(cfg_path)
        from ..agent import AgentConfig, FrenAgent  # delayed import, lightweight now
    ...
    # ---------------- slots / handlers ----------------
    def on_frame(self, frame_bgr, caption: str):
        # import cv2 here (keeps module import light)
        import cv2
